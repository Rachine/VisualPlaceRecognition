...
{
	mexPrintf(
			"Usage(LAB): [predicted_label, accuracy, decision_values/prob_estimates] = predict(testing_label_vector, testing_instance_matrix, model, 'liblinear_options','col')\n"
			...
			for(i=1;i<argc;i++)
			{
				...
				switch(argv[i-1][1])
				{
					...
					case 'q':
						...
						break;
					default:
						mexPrintf("unknown option really\n");
						exit_with_help();
						fake_answer(nlhs, plhs);
						return;
				}
