...
for city_ind = 1 : length(cities)
    ...
    ensuredir(sprintf('%s/%s_%s/%s', config.results_path, config.experiment_id, ...
    ...
    splits_fname = sprintf('%s/split_traincity_%s_metric_%s.mat', ...
    ...
    for split_id = 1 : config.split_num
        ...
        % Save results.
        results_path_ = sprintf('%s/%s_%s/%s', config.results_path, config.experiment_id, ...
                                              city_identifier, metric_str);
        ensuredir(results_path_);
        SaveResults(sprintf('%s/results_split_%d.mat', results_path_, split_id), ...
                    model, test, test_harder, data, data_harder, urban);

        % Plot output results.
        figure_path = sprintf('%s/%s_%s/%s/%d_corr.jpg', ...
            config.results_path, config.experiment_id, city_identifier, metric_str, split_id);
        create_figure_regression(test.test_labels, test.predicted_labels, ...
            sprintf('[c=%.6f, p=%.5f, mse=%.5f, scc=%.5f]', ...
                    model.best_c, model.best_p, test.RMSE, test.R), figure_path);

        figure_path = sprintf('%s/%s_%s/%s/%d_corr_harder.jpg', ...
            config.results_path, config.experiment_id, city_identifier, metric_str, split_id);
        create_figure_regression(test_harder.test_labels, test_harder.predicted_labels, ...
            sprintf('[c=%.6f, p=%.5f, mse=%.5f, scc=%.5f]', model.best_c, model.best_p, ...
            test_harder.RMSE, test_harder.R), figure_path);

        f = fopen(sprintf('%s/%s_%s/%s/%d.html', config.results_path, ...
                          config.experiment_id, city_identifier, metric_str, split_id), 'w');
    ...
    save(sprintf('%s/%s_%s/%s/results.mat', config.results_path, ...
                 config.experiment_id, city_identifier, metric_str), ...
                 'results_split', 'results_split_harder', 'config', 'dataset', ...
                 'dataset_harder', 'urban');
    ...
    rs = results_split;
    fprintf(ff, ' MSE = %2.4f (std: %2.4f), Rsq = %2.4f (std: %2.4f), mRsq = %2.4f]<br/>', ...
            mean(rs(1, :)), std(rs(1, :)), mean(rs(2, :)), std(rs(2, :)), mean(rs(3, :)));
    ...
    for split_id = 1 : config.split_num
        ...
        fprintf(ff, '<img src="%d_corr.jpg"/></a><br/>RMSE = %2.4f, R = %2.4f</td>', ...
                    split_id, results_split(1, split_id), results_split(2, split_id));
        fprintf(ff, '<td><a href="%d.html">', split_id);
        fprintf(ff, '<img src="%d_corr_harder.jpg"/></a><br/>RMSE = %2.4f, R = %2.4f</td></tr>', ...
                    split_id, results_split_harder(1, split_id), results_split_harder(2, split_id));
    end
    fprintf(ff, '</table>');
